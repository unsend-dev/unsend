...
- ```web```: The entire dashboard ```app.unsend.dev``` and our whole email infrastructure. 

- ```marketing```: The marketing infrastructure for mail campaigns.
...
- ```sdk``` package contains software development kit (SDK) that provides reusable code for interacting with APIs or other services.
